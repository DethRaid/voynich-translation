...
def main(sys_argv):
    ...
    try:
        logger.info('trying to get arguments')
        opts, argv = getopt.getopt(sys_argv[1:], "ho:c:",
                                   ["help", "output=", "correction="])
    except getopt.GetoptError, err:
        logger.info('getting arguments failed')
        print str(err)
        usage()
        sys.exit(1)

    logger.info('getting arguments succeeded')
    out_file = "./translated_vecs"
    additional = None
    for opt, val in opts:
        logger.info('checking out option %s with value %s' % opt, val)
        if opt in ("-o", "--ouput"):
            logger.debug('outputtting to filename %s' % val)
            out_file = val
        if opt in ("-c", "--correction"):
            try:
                additional = int(val)
                logger.info('Additional equals %d', additional)
            except ValueError:
                usage(1)
        elif opt in ("-h", "--help"):
            usage(0)
	...
	print str("Wrong number of arguments, you gave us %d but we wanted 4" % len(argv))
	usage(1)
