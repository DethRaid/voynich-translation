...
%            [R,t] = JRMPC(V,X) estimates the Euclidean transformation parameters R,t in order to rigidly register the
%            views in V. V is an M x 1 cell array of views, with each view V{j} j = 1:M represented as a 3 x Nj matrix
%            of (cartesian) coordinates. More specifically a column V{j}(:,i) of an element V{j} in V contains the
%            coordinates [x y z]^T of i-th 3D point of the j-th view. X is a 3 x K matrix of initial centers.
%
